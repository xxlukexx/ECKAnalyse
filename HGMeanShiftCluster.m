...
tic
startTime = cputime;
sampPerSec = nan(1e5, 1);
cnt = 1;
dataPts = single(dataPts);
while numInitPts 
    
    sampPerSec(cnt) = (numPts - numInitPts) / (cputime - startTime);
    if ~isnan(sampPerSec(cnt)) && sampPerSec(cnt) ~= 0
        perc = ((numPts - numInitPts) / numPts) * 100;
        c2 = cnt;
        c1 = cnt - 500;
        if c1 < 1, c1 = 1; end
        secsRem = numInitPts / mean(sampPerSec(c1:c2));
        daysRem = datenum(secsRem / 86400);
        strRem = datestr(daysRem, 'HH:MM:SS');
        fprintf('%d samples remaining %.1f%% (%.2f samples/sec) %s remaining...\n', numInitPts, perc, sampPerSec(cnt), strRem);
        cnt = cnt + 1;
    end
...
%     myOldMean = [0; 0; 0];
%     sqDistToAll = sum((dataPts - myMean) .^ 2);   % temp to init for loop ahead
    
    while true %loop untill convergence

%         if any((myMean - myOldMean) .^ 2 > .00000000001)
%             sqDistToAll = sum(bsxfun(@minus,myMean,dataPts).^2); % dist squared from mean to all points still active
            sqDistToAll = sum((dataPts - myMean) .^ 2);    % ~1.3x faster
