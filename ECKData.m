classdef (ConstructOnLoad) ECKData < handle
    ...
    methods
        ...
        function P = ECKData
            ...
            P.data.FixationBuffer = [];
            P.data.Segments = [];
            P.data.ExtraData = struct;
            
            P.CacheFolder = [tempdir, '_datastreamtemp'];
...
%                 success = false;
            ...
            % Tracker property
            slashes = strfind(sessionPath, filesep);
...
%             if strcmpi(sessionType(sessionPath), 'ET') && loadGaze
        ...
        function [P, success, reason] = SaveSession(P, outputPath)
            ...
            fbFile = findFilename('fixationBuffer', gazePath);
            csvGazeFile = findFilename('gaze', gazePath);
            csvEventFile = findFilename('events', gazePath);
            
            % delete existing files
            if ~isempty(trackerPath) && exist(trackerPath, 'file'), delete(trackerPath); end
            if ~isempty(logPath) && exist(logPath, 'file'), delete(logPath); end
            if ~isempty(mbFile) && exist(mbFile, 'file'), delete(mbFile); end
            if ~isempty(tbFile) && exist(tbFile, 'file'), delete(tbFile); end
            if ~isempty(ebFile) && exist(ebFile, 'file'), delete(ebFile); end
            if ~isempty(fbFile) && exist(fbFile, 'file'), delete(fbFile); end
            if ~isempty(csvGazeFile) && exist(csvGazeFile, 'file'), delete(csvGazeFile); end
            if ~isempty(csvEventFile) && exist(csvEventFile, 'file'), delete(csvEventFile); end
            ...
            if ~isempty(P.FixationBuffer)
                fixBuffer = P.FixationBuffer;
                save(fbFile, 'fixBuffer');
            end
            
            % save CSVs
            ECKSaveLog(sesPath, tempData)
        ...
        function [success, reason] = OverwriteBuffers(P, dontSaveCSVs)
            ...
            try
                save(P.Paths.FixationBuffer, P.FixationBuffer);
            catch ERR
                success = false;
                reason = ERR.message;
            end
        ...
        function [success, reason] = LoadGaze(P)
            ...
            if isempty(mbFile)
                ...
                return
            elseif iscell(mbFile) && length(mbFile) > 1
                success = false;
                reason = 'Multiple main buffers found in /gaze folder.';
                warning(reason)
                return
            end
            ...
            if isempty(tbFile)
                ...
                return
            elseif iscell(tbFile) && length(tbFile) > 1
                success = false;
                reason = 'Multiple time buffers found in /gaze folder.';
                warning(reason)
                return
            end
            ...
            if isempty(ebFile)
                ...
                return
            elseif iscell(ebFile) && length(ebFile) > 1
                success = false;
                reason = 'Multiple event buffers found in /gaze folder.';
                warning(reason)
                return
            end            
        ...
        function [dataQuality] = Quality(P)
             dataQuality = etDataQualityMetric3(P.MainBuffer,...
                 P.TimeBuffer, P.EventBuffer);
        end
        ...
        function P = set.Cached(P, newVal)
            ...
            % check cache folder exists, if not, attempt to create
            if ~exist(P.CacheFolder, 'dir')
                warning('ECKData: cache folder not found, attempting to create...')
                try
                    [suc, msg] = mkdir(P.CacheFolder);
                    if ~suc
                        error('ECKData: attempted to make cache folder, error was:\n%s\n',...
                            msg);
                    end
                catch ERR
                    error('ECKData: attempted to make cache folder, error was:\n%s\n',...
            ...
            % cache of uncache, depending upon current setting
            if newVal ~= P.cached
                if newVal               % cache
                    tmp = P.data;
                    save(P.cacheFile, 'tmp', '-v6')
        ...
        function [newVal] = get.FixationBuffer(P) 
            newVal = P.data.FixationBuffer;
        end        
        ...
        function P = set.FixationBuffer(P, newVal) 
            P.data.FixationBuffer = newVal;
        end        
        ...
        function P = set.GazeLoaded(P, newVal)
