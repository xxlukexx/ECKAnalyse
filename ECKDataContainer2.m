classdef ECKDataContainer2 < handle
    ...
    methods
        ...
        function [P] = ECKDataContainer(loadPath, loadGaze, autoCache)
            ...
            % set temp folder for streaming
            P.streamTempFolder = [pwd, filesep, '_dcstreamtemp'];
            if ~exist(P.streamTempFolder, 'dir')
                [suc, msg, ~] = mkdir(P.streamTempFolder);
                if ~suc
                    error('Attempted to create temp streaming folder, error was:\n%s', msg)
                end
            end
        ...
        function results = LoadFolder(P, folderPath, loadGaze, autoCache)
            ...
            fprintf('Searching folder for data...')
            ...
            % load
            loadCounter = 0;
            st = ECKStatus(sprintf('Loading sessions (%d of %d)...', 1, size(md, 1)));
            numSes = size(md, 1);
            tmp = cell(numSes, 1);
            success = cell(numSes, 1);
            reason = cell(numSes, 1);
            ...
            parfor s = 1:numSes
                tmp{s} = ECKData;
                [~, success{s}, reason{s}] = tmp{s}.Load(md{s, 4}, loadGaze);
                loadCounter = loadCounter + 1;
                parProgress(guid, s, numSes);
                fprintf('Loading sessions %.0f%%...\n', parReadProgress(guid) * 100);
%                 wb = waitbar(s / numSes, wb,...
%                     sprintf('Loading dataset %d of %d...', s, numSes));
            end
            results =...
                [{'Data path', 'Success', 'Reason'}; md(:, 4), success, reason];
            
            wb = waitbar(1, wb, 'Tidying up...');

            for s = 1:numSes
                newIdx = P.AddData(tmp{s});
                if autoCache, P.Data{newIdx}.Cache; end
            end
            
            % remove empties
            P.RemoveEmptyData;
            fprintf('%d sessions loaded.\n', loadCounter)  
            ...
            switch type
                ...
                case 'MAT'
                    ...
                    for d = 1:length(P.Data)
                        ...
                        parProgress(parGUID, d, length(P.Data));
                        fprintf('Saving: %.01f%%...\n', parReadProgress(...
                            parGUID) * 100);
                        
                        % make filename
                        fileName = [outputPath, filesep, P.Data{d}.ParticipantID,...
                            '_', P.Data{d}.TimePoint, '_', P.Data{d}.Battery,...
                            P.Data{d}.GUID, '.eckdata.mat'];
        ...
        function idx = AddData(P, data)
            
            if ~isa(data, 'ECKData')
                error('Must pass an ECKData instance.')
            end
            
            P.data{end + 1} = ECKDuplicateData(data);
...
%                                 d, length(P.Data(filtIdx)));
                            ...
                            try
                                tmp = etDataQualityMetric2(curData);
...
%             rotateXLabels(gca, -90)
        ...
        function P = set.DataStore(P, newVal)
            
            switch newVal
                
                case 'MEMORY'
                    
                    warning('Ability to switch from streaming to memory not yet implemented.')
                    newVal = 'STREAM';
                    
                case 'STREAM'
                    
                    if P.NumData > 0
                        warning('Ability to switch from memory to streaming not yet implemented.')
                        newVal = 'MEMORY';
                    end
                    
                otherwise
                    
                    error('Must specify either MEMORY or STREAM for DataStore property.')
